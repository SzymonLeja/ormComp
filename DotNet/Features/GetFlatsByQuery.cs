...
public class GetFlatsByQuery
{
        ...
        CancellationToken ct)
    {
        ...
        await File.AppendAllTextAsync("results.md", $"## {nameof(GetFlatsByQuery)}\n\nGenerated SQL:\n\n```\n{sql}\n```\n\n", ct);
        ...
        CancellationToken ct)
    {
        ...
        var sql = @"SELECT f.id AS Id, f.description AS Description, f.daily_price_per_person AS DailyPricePerPerson, f.capacity AS Capacity, b.id AS BuildingId, f.flat_number AS FlatNumber
            FROM rental.flats f
        ...
        var flats = await connection.QueryAsync<FlatDto>(sql,new { maxPrice, minCapacity, startDate, endDate });

...
WHERE r.start_date >= '2023-12-31T23:00:00.0000000Z' AND r.end_date <= '2025-05-30T22:00:00.0000000Z'
AND f.building_id IN (SELECT id FROM rental.building WHERE owner_id = 1) 
GROUP BY f.id
...
SELECT f.id AS "Id", (
    ...
    WHERE f.id = r0.flat_id AND r0.start_date >= '2023-12-31T23:00:00.0000000Z' AND r0.end_date <= '2025-05-30T22:00:00.0000000Z') AS "TotalRevenue"
...
ORDER BY (
    ...
    FROM rental.reservations AS r
    WHERE f.id = r.flat_id AND r.start_date >= '2023-12-31T23:00:00.0000000Z' AND r.end_date <= '2025-05-30T22:00:00.0000000Z') DESC;
...
WHERE (
    6371 * acos (
        cos(radians(52)) * cos(radians(a.latitude)) * cos(radians(a.longitude - 23)) +
...
INNER JOIN rental.building AS b ON f.building_id = b.id
INNER JOIN rental.addresses AS a ON b.address_id = a.id
WHERE 6371.0 * acos((0.6156614753256583 * cos((3.1415926535897931 * a.latitude::double precision) / 180.0)) * cos((3.1415926535897931 * a.longitude::double precision) / 180.0 - 0.3839724354387525) + 0.788010753606722 * sin((3.1415926535897931 * a.latitude::double precision) / 180.0)) < 50;
...
FROM rental.flats f
JOIN rental.building b ON f.building_id = b.id
WHERE f.daily_price_per_person <= 50
AND f.capacity >= 2
AND NOT EXISTS (
    SELECT 1 FROM rental.reservations r
    WHERE r.flat_id = f.id
    AND (r.start_date, r.end_date) OVERLAPS ('2023-01-01T00:00:00.0000000Z', '2025-01-31T00:00:00.0000000Z')
)
ORDER BY f.id;

SELECT f.id AS "Id", f.description AS "Description", f.daily_price_per_person AS "DailyPricePerPerson", f.capacity AS "Capacity", f.building_id AS "BuildingId", f.flat_number AS "FlatNumber"
FROM rental.flats AS f
WHERE f.daily_price_per_person <= 50 AND f.capacity >= 2 AND NOT EXISTS (
    SELECT 1
    FROM rental.reservations AS r
    WHERE f.id = r.flat_id AND ((r.start_date <= '2023-01-01T00:00:00.0000000Z' AND r.end_date >= '2025-01-31T00:00:00.0000000Z') OR (r.start_date >= '2024-01-01T00:00:00.0000000Z' AND r.start_date < '2024-01-31T00:00:00.0000000Z') OR (r.end_date > '2024-01-01T00:00:00.0000000Z' AND r.end_date <= '2024-01-31T00:00:00.0000000Z')))
ORDER BY f.id;
...
COPY (SELECT * FROM pg_stat_statements WHERE queryid in (-417404113517924016, -6051908327733557471, -6350233935968508531, -1616123222825357101, 2925326113394445949, 5790089386458641856, -2639351766443624826, 1566226529174231237, 3075921541462290673, 3851029712228439361, -4818722781088261692, -507885921649201663, -6456960838080312881)) TO 'put/path/here' DELIMITER ';' CSV HEADER;
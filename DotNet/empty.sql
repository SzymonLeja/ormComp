...
WHERE (
    ...
    WHERE u.id = b.owner_id AND a.city = 'test') = 5;
...
SELECT r.id AS Id, r.start_date AS StartDate, r.end_date AS EndDate, r.guest_number AS GuestNumber, r.total_cost AS TotalCost
            ...
            WHERE r.reserved_by_id = 23095832;
...
WHERE r.start_date >= '2033-12-31T23:00:00.0000000Z' AND r.end_date <= '2034-05-30T22:00:00.0000000Z'
AND f.building_id IN (SELECT id FROM rental.building WHERE owner_id = 543543) 
GROUP BY f.id
...
SELECT f.id AS "Id", (
    ...
    WHERE f.id = r0.flat_id AND r0.start_date >= '2033-12-31T23:00:00.0000000Z' AND r0.end_date <= '2034-05-30T22:00:00.0000000Z') AS "TotalRevenue"
FROM rental.flats AS f
INNER JOIN rental.building AS b ON f.building_id = b.id
WHERE b.owner_id = 543543
ORDER BY (
    SELECT COALESCE(sum(r.total_cost), 0)
    FROM rental.reservations AS r
    WHERE f.id = r.flat_id AND r.start_date >= '2033-12-31T23:00:00.0000000Z' AND r.end_date <= '2034-05-30T22:00:00.0000000Z') DESC;
...
WHERE (
    6371 * acos (
        cos(radians(60)) * cos(radians(a.latitude)) * cos(radians(a.longitude - 30)) +
        sin(radians(60)) * sin(radians(a.latitude))
    )
) < 0.1;
...
FROM rental.flats f
JOIN rental.building b ON f.building_id = b.id
WHERE f.daily_price_per_person <= 150
AND f.capacity >= 100
AND NOT EXISTS (
...
SELECT f.id AS "Id", f.description AS "Description", f.daily_price_per_person AS "DailyPricePerPerson", f.capacity AS "Capacity", f.building_id AS "BuildingId", f.flat_number AS "FlatNumber"
FROM rental.flats AS f
WHERE f.daily_price_per_person <= 150 AND f.capacity >= 100 AND NOT EXISTS (
    SELECT 1
    FROM rental.reservations AS r
...
COPY (SELECT * FROM pg_stat_statements WHERE queryid in (5669944252977757226, -6051908327733557471, 2925326113394445949, 5790089386458641856, -2639351766443624826, 1566226529174231237, 3075921541462290673, 3851029712228439361, -4818722781088261692, 7964517876084347408, -507885921649201663, 2568884615005532003, -6390807139547292995)) TO 'put/path/here' DELIMITER ';' CSV HEADER;